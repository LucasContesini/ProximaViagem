...
// Simulação da lógica do backend Go
const getDailyDestination = async (language = 'pt') => {
  // Dados de exemplo - em produção, aqui chamaria a API do Groq
  const destinations = {
    pt: {
      id: `dest-${Date.now()}`,
      name: "Gramado",
  ...
  return destinations[language] || destinations.pt;
};
...
exports.handler = async (event, context) => {
  ...
  try {
    ...
    const acceptLanguage = event.headers['accept-language'] || 'pt';
    let language = 'pt';
    ...
    // Obter destino
    const destination = await getDailyDestination(language);
